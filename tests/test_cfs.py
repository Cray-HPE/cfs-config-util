...
from cfs_config_util.vcs import VCSError
...
class TestCFSConfigurationLayer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_vcs_repo_cls = patch('cfs_config_util.cfs.VCSRepo').start()
        self.mock_vcs_repo = self.mock_vcs_repo_cls.return_value

        self.mock_datetime = patch('cfs_config_util.cfs.datetime', wraps=datetime.datetime).start()
    ...
    def test_construct_name_playbook_and_branch(self):
        ...
        name = CFSConfigurationLayer.construct_name(self.product, playbook=self.playbook,
                                                    branch='integration')
        self.assertEqual(f'{self.product}-{self.playbook[:-4]}-integration-{self.expected_timestamp}',
                         name)
...
class TestCFSConfigurationLayerFromProduct(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.clone_url = f'http://vcs.system.domain/vcs/cray/{self.product_name}-config-management.git'
        self.product_commit = 'abcdef7654321'

        self.api_gw_host = patch('cfs_config_util.cfs.API_GW_HOST', 'api-gw-service')
        self.expected_clone_url = f'http://{API_GW_HOST}/vcs/cray/{self.product_name}-config-management.git'

        self.mock_product_catalog_cls = patch('cfs_config_util.cfs.ProductCatalog').start()
    ...
    def test_product_defaults(self):
        ...
        layer = CFSConfigurationLayer.from_product_catalog(self.product_name,
    ...
    def test_product_no_version_with_commit_hash(self):
        ...
        layer = CFSConfigurationLayer.from_product_catalog(self.product_name, commit=alternate_commit)
    ...
    def test_product_catalog_failure(self):
        ...
        with self.assertRaisesRegex(CFSConfigurationError, err_regex):
            CFSConfigurationLayer.from_product_catalog(self.product_name)
...
class TestCFSConfiguration(unittest.TestCase):
    ...
    @patch('cfs_config_util.cfs.CFSConfigurationLayer.from_cfs')
    ...
    def test_save_to_cfs(self):
        ...
        with patch('cfs_config_util.cfs.CFSConfiguration') as mock_cfs_config_cls:
